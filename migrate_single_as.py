...
class ZabbixAPI:
    def __init__(self, api_url: str, timeout_sec: int = 120) -> None:
    ...
    def call(self, method: str, params: Dict[str, Any]) -> Any:
        payload: Dict[str, Any] = {"jsonrpc": "2.0", "method": method, "params": params, "id": self._id}
        self._id += 1
...
class GrafanaAPI:
    def __init__(self, base_url: str, username: str, password: str, token: str, timeout_sec: int = 120) -> None:
    ...
    def get(self, path: str, params: Optional[dict] = None) -> Any:
        r = self.s.get(f"{self.base_url}{path}", params=params, timeout=self.timeout, verify=False)
        r.raise_for_status()
        return r.json()

    def post(self, path: str, payload: dict) -> Any:
        r = self.s.post(f"{self.base_url}{path}", json=payload, timeout=self.timeout, verify=False)
        r.raise_for_status()
        return r.json()
...
def main() -> int:
    ...
    # --- Grafana ---
    print("\n=== Grafana migration ===")
    gapi = GrafanaAPI(GRAFANA_URL, GRAFANA_USER, GRAFANA_PASSWORD, GRAFANA_TOKEN)
